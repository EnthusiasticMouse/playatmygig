      ...
      <>
        ...
        <div className={styles.profileBackground}>
          <Image
            ...
            src={`http://localhost:3001/images/${props.userID}/0`}
          />
        </div>
        <div id={styles.profilePic}>
          <img
            alt="profile picture"
            src={`http://localhost:3001/avatars/${props.userID}`}
          />{" "}
        </div>
        ...
        <div className={styles.loginBox}>
          <form method="POST" action={"/api/update/user"}>
            ...
            {props.isVenue ? (
              <>
                ...
                <input
                  type="text"
                  placeholder={props.payment?.toString() || "none"}
                  id="payment"
                  name="payment"
                  value={payment}
          ...
          <form
            ...
            method="post"
            action="http://localhost:3001/update/avatar/"
          >
          ...
          <form
            encType="multipart/form-data"
            method="post"
            action="http://localhost:3001/update/image/"
          >
          ...
          <div className={styles.links}>
            <Link
              href={`http://localhost:3000/api/update/changeType/${props.isVenue}`}
            >
              <a>
                {props.isVenue ? <>Change To Artist</> : <> Change To Venue</>}
              </a>
            </Link>
            <Link
              href={`http://localhost:3000/api/update/publish/${props.published}`}
            >
              <a>{props.published ? <>Unpublish</> : <> Publish</>}</a>
            </Link>
          </div>
        </div>
